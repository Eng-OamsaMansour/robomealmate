...
return_type DiffDriveArduino::configure(const hardware_interface::HardwareInfo & info)
{
  ...
  std::string nm = info_.hardware_parameters["num_wheel_pairs"];
  RCLCPP_INFO(logger_, nm);
  cfg_.front_left_wheel_name = info_.hardware_parameters["front_left_wheel_name"];
  RCLCPP_INFO(logger_, cfg_.front_left_wheel_name);
  cfg_.front_right_wheel_name = info_.hardware_parameters["front_right_wheel_name"];
  RCLCPP_INFO(logger_, cfg_.front_right_wheel_name);
  cfg_.back_left_wheel_name = info_.hardware_parameters["back_left_wheel_name"];
  RCLCPP_INFO(logger_, cfg_.back_left_wheel_name);
  cfg_.back_right_wheel_name = info_.hardware_parameters["back_right_wheel_name"]; 
  RCLCPP_INFO(logger_, cfg_.back_right_wheel_name); 
  cfg_.loop_rate = std::stof(info_.hardware_parameters["loop_rate"]);
  RCLCPP_INFO(logger_, std::to_string(cfg_.loop_rate));
  cfg_.device = info_.hardware_parameters["device_name"];
  RCLCPP_INFO(logger_, cfg_.device);
  cfg_.baud_rate = std::stoi(info_.hardware_parameters["baud_rate"]);
  RCLCPP_INFO(logger_, std::to_string(cfg_.baud_rate));
  cfg_.timeout = std::stoi(info_.hardware_parameters["timeout"]);
  RCLCPP_INFO(logger_, std::to_string(cfg_.timeout));
  cfg_.enc_counts_per_rev = std::stoi(info_.hardware_parameters["enc_counts_per_rev"]);
  RCLCPP_INFO(logger_, std::to_string(cfg_.enc_counts_per_rev));
...
return_type DiffDriveArduino::start()
{
 ...
 // arduino_.setPidValues(30, 20, 0, 100);
  arduino_.setMotorValues(30,30,30,30);
...
hardware_interface::return_type DiffDriveArduino::read()
{
  ...
  RCLCPP_INFO(logger_, "e" + std::to_string(f_l_wheel_.enc) + " " + std::to_string(f_r_wheel_.enc) + " " + std::to_string(b_l_wheel_.enc) + " " + std::to_string(b_r_wheel_.enc));
...
hardware_interface::return_type DiffDriveArduino::write()
{
  ...
  RCLCPP_INFO(logger_, "m" + std::to_string(f_l_wheel_.cmd / f_l_wheel_.rads_per_count / cfg_.loop_rate)+ " " + std::to_string(f_r_wheel_.cmd / f_r_wheel_.rads_per_count / cfg_.loop_rate) + " " + std::to_string(b_l_wheel_.cmd / b_l_wheel_.rads_per_count / cfg_.loop_rate) + " " + std::to_string(b_r_wheel_.cmd / b_r_wheel_.rads_per_count / cfg_.loop_rate));
  return return_type::OK;
